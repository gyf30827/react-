...
function processDispatchQueueItemsInOrder(
  ...
  if (inCapturePhase) {
    for (let i = dispatchListeners.length - 1; i >= 0; i--) {
      ...
      // 该函数会在自定义合成对象调用 stopPropagation 函数设置为已知返回true的函数，
      // 结束后续函数的调用
...
export function listenToNonDelegatedEvent(
  ...
  // 此函数会在 dom元素的 【internalEventHandlersKey】属性上生成一个Set集合
  // 用来存储已绑定的事件即 【listenerSetKey】
  const listenerSet = getEventListenerSet(targetElement);
  // 获取ListenerKey 该key 是根据事件名和冒泡阶段还是捕获阶段拼接而成
  const listenerSetKey = getListenerSetKey(
    domEventName,
    isCapturePhaseListener,
  );
  // dom 上没有绑定过此事件 即调用 addTrappedEventListener 绑定事件
  // 此处  isCapturePhaseListener 一直未false 即绑定冒泡阶段事件
...
export function listenToNativeEventForNonManagedEventTarget(
  ...
  console.log('listenToNativeEventForNonManagedEventTarget');
...
export function dispatchEventForPluginEventSystem(
  ...
  customLog(
    nativeEvent,
    'dispatchEventForPluginEventSystem.eventPhase',
    nativeEvent.eventPhase === 1 ? '捕获阶段' : '冒泡阶段',
  );
  customLog(
    nativeEvent,
    'dispatchEventForPluginEventSystem.eventSystemFlags',
    eventSystemFlags,
    nativeEvent.target,
  );
  /**
   * IS_EVENT_HANDLE_NON_MANAGED_NODE = 1 二进制 001
   * IS_NON_DELEGATED = 2 二进制 010
   * 捕获阶段
   *  eventSystemFlags =  4 二进制 100
   * 冒泡阶段
   *  eventSystemFlags = 0 二进制 000
   */
  if (
    (eventSystemFlags & IS_EVENT_HANDLE_NON_MANAGED_NODE) === 0 &&
    (eventSystemFlags & IS_NON_DELEGATED) === 0
  ) {
    customLog(nativeEvent, 'dispatchEventForPluginEventSystem', 1);
...
export function accumulateSinglePhaseListeners(
  ...
  // 获取reactEventName 冒泡阶段直接使用 reactName  否则使用 captureName
