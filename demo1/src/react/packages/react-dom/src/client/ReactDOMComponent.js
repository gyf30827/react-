...
export function setInitialProperties(
  ...
  let props: Object;
  // 根据不同的dom类型，将不支持事件委托的事件绑定到具体的dom元素上
  // 初始化一些默认属性属性
  switch (tag) {
    case 'dialog':
...
export function diffProperties(
  ...
  console.log('diffProperties', lastRawProps, nextRawProps);
  if (__DEV__) {
  ...
  for (propKey in lastProps) {
    ...
    // 旧的属性在新的props中不存在，即删除了某属性
    console.log('diffProperties', '删除了属性', propKey);
    if (propKey === STYLE) {
      // 如果是style 则将原有的 style 值全部清空赋值到 styleUpdates
  ...
  for (propKey in nextProps) {
    ...
    if (propKey === STYLE) {
      ...
      // 遍历旧的样式 将删除了的样式赋值到 styleUpdates 上
      if (lastProp) {
        ...
        // 遍历新的样式，将有变化的样式更新到 styleUpdates 上
