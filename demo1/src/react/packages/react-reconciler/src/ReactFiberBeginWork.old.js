...
function forceUnmountCurrentAndReconcile(
  ...
  // passing null.
  // 将 nextChildren 传null 将所有的 current.child 卸载
  workInProgress.child = reconcileChildFibers(
    workInProgress,
    current.child,
    null,
  ...
  // identities match.
  // 将 current.child 传为null 强制重新挂载 nextChildren
  workInProgress.child = reconcileChildFibers(
    workInProgress,
    null,
    nextChildren,
...
function updateMemoComponent(
  ...
  // 是否存在待更新的优先级包含context的变化
  const hasScheduledUpdateOrContext = checkScheduledUpdateOrContext(
    current,
    renderLanes,
  );
  // 不存在 存在待更新的优先级包含context的变化
  if (!hasScheduledUpdateOrContext) {
    const prevProps = currentChild.memoizedProps;
    // Default to shallow comparison
    let compare = Component.compare;
    compare = compare !== null ? compare : shallowEqual;
    // 对props进行比较如果没有变化，符合优化策略直接返回
...
function updateOffscreenComponent(
  ...
  console.log('updateOffscreenComponent');
...
// 如果新增或者删除ref将 workInProgress.flags 打上 Ref 变化 的标识
...
function updateClassComponent(
  ...
  const instance = workInProgress.stateNode;
  let shouldUpdate;
  // mount 时 instance 为null
  if (instance === null) {
    console.log('updateClassComponent', 'mount', '创建instance');
    resetSuspendedCurrentOnMountInLegacyMode(current, workInProgress);

    // In the initial pass we might need to construct the instance.
    //  创建 类组件实例
    constructClassInstance(workInProgress, Component, nextProps);
    // 初始化实例 上  props state 等信息
    mountClassInstance(workInProgress, Component, nextProps, renderLanes);
    shouldUpdate = true;
  } else if (current === null) {
    // 当该class组件时 ErrorBoundry时，子节点抛出异常，会回溯到此节点重新渲染
    // 此时 instance 已经存在，但是 依然处于mount阶段
...
function finishClassComponent(
  ...
  ReactCurrentOwner.current = workInProgress;
  let nextChildren;
  // 如果捕获到到了报错信息但是 没有 getDerivedStateFromError 生命周期来捕获错误信息
  // 则将 nextchildren 设置为null
  if (
    didCaptureError &&
...
function updateHostRoot(current, workInProgress, renderLanes) {
  ...
  // console.log('workLoop', 'beginWork', 'updateHostRoot', current);
  if (current === null) {
  ...
  cloneUpdateQueue(current, workInProgress);
  // console.log('workLoop', 'beginWork', 'updateHostRoot.nextProps', nextProps);
  processUpdateQueue(workInProgress, nextProps, null, renderLanes);
  // console.log(
  //   'workLoop',
  //   'beginWork',
  //   'updateHostRoot.workInProgress.memoizedState.element',
  ...
  //   'workLoop',
  //   'beginWork',
  //   'updateHostRoot.nextChildren&prevChildren',
  //   nextChildren,
  //   prevChildren,
  // );
  // 混合应用
  if (supportsHydration && prevState.isDehydrated) {
    ...
    // console.log('workLoop beginWork updateHostRoot', 6666);
    // Root is not dehydrated. Either this is a client-only root, or it
    // already hydrated.
    resetHydrationState();
    // 如果前后children 相同则直接进入
...
function updateHostComponent(
  ...
  // console.log('workLoop', 'updateHostComponent', current);
  // mount阶段
  if (current === null) {
  ...
  // 如果子节点可以是文本节点，我们不需要为其创建Fiber节点， 所以 nextChildren 为 null
  if (isDirectTextChild) {
    ...
    // 如果节点原来的子Fiber 由非文本节点变为文本节点 则将此Fiber 打上 内容需要重置（ContentReset）的标识
...
function mountLazyComponent(
  ...
  const init = lazyComponent._init;
  // 获取加载的结果,此时有两种情况
  // 一种情况是组件还未加载，此时会调用加载函数，进行异步加载，此时会在此抛出一个 正在加载的 promise
  // 另外一种情况是已经加载，此处获得已经加载到的 组件
...
function shouldRemainOnFallback(
  ...
  // suspenseContext 中不存在 ForceSuspenseFallback 时， 为true
...
function updateSuspenseComponent(current, workInProgress, renderLanes) {
  ...
  if (
    ...
    if (
      ...
      if (
        ...
        // 向 全局的 suspenseContext 添加 InvisibleParentSuspenseContext
  ...
  // 添加 向 suspenseContext添加  SubtreeSuspenseContextMask
  suspenseContext = setDefaultShallowSuspenseContext(suspenseContext);
  // 将 suspenseContext 添加到全局队列 valueStack 中
  ...
  if (current === null) {
    ...
    // 第一次进来时 showFallback 为 false
    // 第二次进来时 showFallback 为 true
    if (showFallback) {
...
// 创建一个 OffscreenComponent 类型的子Fiber
  ...
  if (enableLazyContextPropagation) {
    ...
    if (oldProps !== null) {
      ...
      if (is(oldValue, newValue)) {
        ...
        // children也没有变化并且没有 context 变化 符合优化策略直接
        // 复用原有节点
        if (
        ...
        // them to update.
        // context.provider的value 有变化
        // 遍历子节点将依赖此context.provider 的子组件的lanes 已经childLanes都打上 renderLanes
        // 标识，该节点在此次更新中存在需要处理的更新
...
function bailoutOnAlreadyFinishedWork(
  ...
  // console.log('bailoutOnAlreadyFinishedWork', current);
  // 更新阶段
  if (current !== null) {
  ...
  // 将 workInProgress.lanes 全部设置为跳过的lanes
  markSkippedUpdateLanes(workInProgress.lanes);

  // Check if the children have any pending work.
  // 子Fiber的待更新事件优先级不足，后续子Fiber无需处理
...
function checkScheduledUpdateOrContext(
  ...
  // 如果current.lanes 包含 renderLanes 则返回true
  // 即当前Fiber current 存在待更新的优先级
  // updateLanes 不包含 renderLanes 时返回true
  if (includesSomeLane(updateLanes, renderLanes)) {
    return true;
  }
  // No pending update, but because context is propagated lazily, we need
  // to check for a context change before we bail out.
  // 没有待更新队列，但是需要检查 context 是否有更新
  if (enableLazyContextPropagation) {
...
function beginWork(
  ...
  // console.log('workLoop', 'beginWork.current', current);
  if (current !== null) {
    ...
    if (
      ...
      if (
        ...
        // 和错误边界相关
        //如果这是第二次通过的错误或悬念边界，那里
        //可能没有在' current '上安排工作，所以我们检查这个标志。
        (workInProgress.flags & DidCapture) === NoFlags
      ) {
        // console.log(
        //   66666,
