...
function commitBeforeMutationEffectsOnFiber(finishedWork: Fiber) {
  ...
  // 如果存在 Snapshot 即存在 getSnapshotBeforeUpdate 生命周期时会在Fiber节点上打上此标识
...
function commitHookEffectListUnmount(
  // 传入的需要执行的 标识
  // useEffect时： HookPassive | HookHasEffect,
  flags: HookFlags,
  finishedWork: Fiber,
  nearestMountedAncestor: Fiber | null,
) {
  ...
  if ((finishedWork.flags & LayoutMask) !== NoFlags) {
    switch (finishedWork.tag) {
      ...
      case SimpleMemoComponent: {
        if (
          ...
          if (
            ...
            // HookLayout代表是 useLayoutEffect hooks
            // 调用  commitHookEffectListMount 函数，执行useLayoutEffect 函数的挂载函数
          ...
          if (!offscreenSubtreeWasHidden) {
            ...
            if (current === null) {
              ...
              if (
                ...
                // 执行 componentDidUpdate 生命周期，注意第三个参数
                // instance.__reactInternalSnapshotBeforeUpdate,
                // 是执行getSnapshotBeforeUpdate 周期时返回的值
                instance.componentDidUpdate(
                  prevProps,
                  prevState,
                  instance.__reactInternalSnapshotBeforeUpdate,
      ...
      case HostComponent: {
        ...
        // mount阶段，dom元素挂载后，执行autoFocus 以及 图片的加载
...
function detachFiberAfterEffects(fiber: Fiber) {
  ...
  // `deletedTreeCleanUpLevel` is undefined. = 3
  if (!(deletedTreeCleanUpLevel >= 2)) {
    ...
    // 清空 stateNode， 对于dom对应的fiber来说,是dom实例
    fiber.stateNode = null;
...
function insertOrAppendPlacementNodeIntoContainer(
  ...
  if (isHost) {
    ...
    if (before) {
      ...
      // 直接向父元素插入stateNode parent.appendChild(stateNode)
...
function commitDeletionEffects(
  ...
  // true
  if (supportsMutation) {
    //我们只删除了最上面的Fiber，但是我们需要递归它的子节点查找所有终端节点。
    // We only have the top Fiber that was deleted but we need to recurse down its
    // children to find all the terminal nodes.
    //从父节点递归删除所有主机节点，分离refs，清理挂载的布局效果，并调用componentWillUnmount。
...
function recursivelyTraverseMutationEffects(
  ...
  // 遍历当前节点下需要删除的节点 deletions
  // 执行删除节点的上副作用，useEffect useLayoutEffect 的卸载函数
      ...
      if (supportsMutation) {
        ...
        if (flags & Update) {
          ...
          if (instance != null) {
            ...
            // 如果dom属性存在变化，在completeWork阶段会将变化赋值到 updateQueue 上
...
function commitLayoutEffects_begin(
  ...
  while (nextEffect !== null) {
    ...
    // Suspense 组件的 tag 为 OffscreenComponent
    if (
      enableSuspenseLayoutEffectSemantics &&
