...
function createClassErrorUpdate(
  ...
  const update = createUpdate(NoTimestamp, lane);
  // 将update打上CaptureUpdate 标签
  update.tag = CaptureUpdate;
  // 获取到 getDerivedStateFromError 函数 将其添加到
  const getDerivedStateFromError = fiber.type.getDerivedStateFromError;
  // 如果存在 getDerivedStateFromError 函数 将 其添加到 update.payload函数中执行
  ...
  // 如果已经创建了类组件实例并且存在componentDidCatch生命周期
  // 将上面添加update.callback 重新赋值，并在函数内调用 componentDidCatch
  // 生命周期函数
...
// 给当前的promise 添加一个 then(pingSuspendedRoot), 回调函数为 pingSuspendedRoot
...
// 重置函数组件的  updateQueue memoizedState lanes
...
function getNearestSuspenseBoundaryToCapture(returnFiber: Fiber) {
  ...
  // 当首次渲染 时会将 InvisibleParentSuspenseContext 添加到 suspenseStackCursor.current
...
function throwException(
  ...
  console.log('main', 'throwException', '抛出异常了', sourceFiber);
  if (enableUpdaterTracking) {
    if (isDevToolsPresent) {
      // If we have pending work still, restore the original updaters
  ...
  // as an error.
  // 从抛出异常组件向上查找 查找 ErrorBoundry 直至找到根节点
