...
export function buildCreateSlice({ creators }: BuildCreateSliceConfig = {}) {
  ...
  >(
    ...
    // 存储该 slice 的信息
    // 包括 action reducer 等信息
    const context: ReducerHandlingContext<State> = {
      // 该对象存储的为 reducer.name 和reducer 的映射
    ...
    const contextMethods: ReducerHandlingContextMethods<State> = {
      // 添加一个case 放到 context.sliceCaseReducersByType
      addCase(
    ...
    reducerNames.forEach((reducerName) => {
      ...
      const reducerDetails: ReducerDetails = {
        reducerName,
        // 拼接 name 和 reducerName 为 action
    ...
    // 通过 createReducer 创建 reducer 执行函数
    // 将我们获取到的所有  reducer  添加到case 中
    function buildReducer() {
      ...
      // 如果存在 extraReducers 并且是函数的话则调用
      // executeReducerBuilderCallback 函数，执行
      // extraReducers 获取其返回的 配置
      const [
        extraReducers = {},
        actionMatchers = [],
      ...
      // 将reudcers 和 extraReducers 合并获得该 slice 对应的所有的 reucer
      const finalCaseReducers = {
        ...extraReducers,
        ...context.sliceCaseReducersByType,
      }
      // 调用createReducer 函数构建reducer
    ...
    // 此函数为该slice注册到 redux 的 reducer
    // 当 触发 dispatch 时，会执行此函数
...
function handleNormalReducerDefinition<State>(
  ...
  console.log('redux-toolkit', 'createSlice', type)
...
// 当 reducerDefinition 是一个异步thunk 时
function handleThunkCaseReducerDefinition<State>(
  ...
  // 获取关于该异步thunk的所有配置，此时 reducerDefinition 为个对象
  // payloadCreator 这个为我们调用 asyncThunk 函数时传入的函数
  const { payloadCreator, fulfilled, pending, rejected, settled, options } =
    reducerDefinition
  // cAT 实际即为 createAsyncThunk 函数，调用函数生成一个异步的thunk
  // 此thunk 为一个action
